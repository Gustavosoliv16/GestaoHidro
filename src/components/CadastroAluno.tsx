...
import { salvarAlunoCompleto, atualizarAlunoCompleto } from "../services/AlunoService";
import {
  agendarReposicao,
  cancelarReposicao,
...
function exigeResponsavel(nomeModalidade: string): boolean {
  const norm = (nomeModalidade ?? "")
    ...
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "");
  return norm.includes("infantil") || norm.includes("bebe") || norm.includes("bebê");
}
...
export default function NovoAlunoModal({
  ...
  const [nomeModalidadeSelecionada, setNomeModalidadeSelecionada] = useState("");
  const [diaVencimento, setDiaVencimento] = useState<number | null>(() => new Date().getDate());
  ...
  const [turmaSelecionadaReposicao, setTurmaSelecionadaReposicao] = useState<number | null>(null);
  ...
  const idAlunoEditando = alunoParaEditar?.id_aluno ?? alunoParaEditar?.id ?? null;
  ...
  const handleAgendarReposicao = async () => {
    if (!turmaSelecionadaReposicao || !dataReposicao) {
      toast.current?.show({ severity: "warn", summary: "Aviso", detail: "Selecione a turma e a data da reposição." });
      return;
    }
  ...
  useEffect(() => {
    const found = opcoesModalidades.find((m: any) => m.id_modalidade === modalidade);
  ...
  const executarSalvar = async () => {
    const strip = (s: string | null | undefined) => (s ? s.replace(/\D/g, "") : "");
    ...
    const dadosParaSalvar = {
      ...
      responsavel: nomeResp.trim()
        ? { nome: nomeResp.trim(), telefone: telefoneResp, documento: documentoResp }
        : undefined,
    };
    ...
    try {
      if (alunoParaEditar && (alunoParaEditar.id_aluno || alunoParaEditar.id)) {
        ...
        setTimeout(() => { aoSalvar({ ...alunoParaEditar, ...dadosParaSalvar }); aoFechar(); }, 500);
      } else {
        await salvarAlunoCompleto(dadosParaSalvar);
        toast.current?.show({ severity: "success", summary: "Sucesso", detail: "Aluno salvo!", life: 3000 });
        const novoAluno: Aluno = {
          id: Date.now(), nome, telefone, modalidade: modalidade as any,
          diaVencimento: diaVencimento === null ? 0 : Math.trunc(diaVencimento),
          nascimento, documento, endereco,
          valorMensalidade: valorMensalidade || 0, horariosFixos,
        };
        setTimeout(() => { aoSalvar(novoAluno); aoFechar(); }, 500);
      }
    } catch (erro) {
      console.error("Erro ao salvar:", erro);
      toast.current?.show({ severity: "error", summary: "Erro", detail: "Erro ao salvar o aluno.", life: 4000 });
    }
  };
  ...
  return (
    <div className="flex flex-column gap-4 w-full">
      ...
      <div>
        ...
        <p className="text-600 text-sm mt-1 mb-0">Preencha os dados do aluno abaixo</p>
      </div>
      ...
      <Card className="surface-card border-1 surface-border p-0">
        <div className="p-4">
          <div className="grid">
            ...
            <div className="col-12">
              <label htmlFor="nome" className="font-semibold block mb-2 text-900">Nome Completo</label>
              <InputText id="nome" value={nome} placeholder="Digite o nome completo"
                onChange={(e) => setNome(e.target.value)} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label htmlFor="tel" className="font-semibold block mb-2 text-900">Telefone</label>
              <InputMask id="tel" value={telefone} mask="(99) 99999-9999" placeholder="(00) 00000-0000"
                onChange={(e: InputMaskChangeEvent) => setTelefone(e.target.value ?? "")} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label htmlFor="documento" className="font-semibold block mb-2 text-900">Documento (CPF)</label>
              <InputMask id="documento" value={documento} mask="999.999.999-99" placeholder="000.000.000-00"
                onChange={(e: InputMaskChangeEvent) => setDocumento(e.target.value ?? "")} className="w-full" />
            </div>

            <div className="col-12">
              <label htmlFor="endereco" className="font-semibold block mb-2 text-900">Endereço</label>
              <InputText id="endereco" value={endereco} placeholder="Rua, avenida..."
                onChange={(e) => setEndereco(e.target.value)} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label htmlFor="numero" className="font-semibold block mb-2 text-900">Número</label>
              <InputText id="numero" value={numero} placeholder="Número"
                onChange={(e) => setNumero(e.target.value)} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label htmlFor="bairro" className="font-semibold block mb-2 text-900">Bairro</label>
              <InputText id="bairro" value={bairro} placeholder="Bairro"
                onChange={(e) => setBairro(e.target.value)} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label htmlFor="cidade" className="font-semibold block mb-2 text-900">Cidade</label>
              <InputText id="cidade" value={cidade} placeholder="Cidade"
                onChange={(e) => setCidade(e.target.value)} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label htmlFor="nascimento" className="font-semibold block mb-2 text-900">Data de Nascimento</label>
              <InputMask id="nascimento" value={nascimento} mask="99/99/9999" placeholder="dd/mm/yyyy"
                onChange={(e: InputMaskChangeEvent) => setNascimento(e.target.value ?? "")} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label className="font-semibold block mb-2 text-900">Modalidade</label>
              <Dropdown value={modalidade} options={opcoesModalidades}
                onChange={(e) => setModalidade(e.value)}
                optionLabel="modalidade" optionValue="id_modalidade"
                placeholder="Selecione..." className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label className="font-semibold block mb-2 text-900">Dia Vencimento</label>
              <InputNumber value={diaVencimento}
                onValueChange={(e) => { const v = e.value ?? null; setDiaVencimento(v === null ? null : Math.trunc(v)); }}
                min={1} max={31} className="w-full" maxFractionDigits={0} placeholder="5" />
            </div>

            <div className="col-12 md:col-6">
              <label className="font-semibold block mb-2 text-900">Mensalidade</label>
              <InputNumber value={valorMensalidade} onValueChange={(e) => setValorMensalidade(e.value ?? null)}
                mode="currency" currency="BRL" locale="pt-BR" className="w-full" placeholder="R$ 0,00" />
            </div>
        ...
        <div className="p-4">
          <div className="flex align-items-center gap-2 mb-3">
            ...
            {precisaResponsavel ? (
              <span className="text-xs font-semibold px-2 py-1 border-round"
          ...
          <div className="grid">
            <div className="col-12">
              <label className="font-semibold block mb-2 text-900">
                Nome do Responsável {precisaResponsavel && <span className="text-red-500">*</span>}
              </label>
              <InputText value={nomeResp} placeholder="Nome completo do responsável"
                onChange={(e) => setNomeResp(e.target.value)} className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label className="font-semibold block mb-2 text-900">
                CPF do Responsável {precisaResponsavel && <span className="text-red-500">*</span>}
              </label>
              <InputMask value={documentoResp} mask="999.999.999-99" placeholder="000.000.000-00"
                onChange={(e: InputMaskChangeEvent) => setDocumentoResp(e.target.value ?? "")}
                className="w-full" />
            </div>

            <div className="col-12 md:col-6">
              <label className="font-semibold block mb-2 text-900">Telefone do Responsável</label>
              <InputMask value={telefoneResp} mask="(99) 99999-9999" placeholder="(00) 00000-0000"
                onChange={(e: InputMaskChangeEvent) => setTelefoneResp(e.target.value ?? "")}
                className="w-full" />
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-content-end gap-2 p-4">
          <Button label="Cancelar" icon="pi pi-times" severity="secondary" onClick={aoFechar} />
            ...
            {!alunoAtivo ? (
              ...
              <div className="grid">
                <div className="col-12 md:col-5">
                  <label className="font-semibold block mb-2 text-900 text-sm">Turma</label>
                  <Dropdown
                ...
                <div className="col-12 md:col-3">
                  <label className="font-semibold block mb-2 text-900 text-sm">Data</label>
                  <input
                    type="date"
                    value={dataReposicao}
                ...
                <div className="col-12 md:col-4">
                  <label className="font-semibold block mb-2 text-900 text-sm">Observação (opcional)</label>
                  <InputText
                ...
                <div className="col-12 flex justify-content-end">
                  <Button
                    ...
                    icon={salvandoReposicao ? "pi pi-spin pi-spinner" : "pi pi-plus"}
                    className="p-button-sm p-button-outlined"
                    disabled={salvandoReposicao || !turmaSelecionadaReposicao || !dataReposicao}
                    ...
                    return (
                      <div key={r.id_reposicao}
                        ...
                        <div className="flex align-items-center gap-2">
                          ...
                          {r.observacao && (
                            <span className="text-xs text-400">({r.observacao})</span>
                          )}
                        </div>
                        {r.status === "AGENDADA" && (
                          <Button
                            ...
                            onClick={() => handleCancelarReposicao(r.id_reposicao)}
                          />
                        )}
                      </div>
                    );
